                         ...
                         0, 0)  // Default priority and stack size
{
  ...
  createParam(MCS2AMPLENString, asynParamInt32, &this->amplen_);
...
asynStatus MCS2Controller::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
  ...
  else if (function == amplen_) {
    /* send amplifier enable command */
    temp = (value>0) ? 1:0;
    sprintf(pAxis->pC_->outString_, ":CHAN%d:AMPL %d", pAxis->axisNo_, temp);
    status = pAxis->pC_->writeController();
  }
  else {
